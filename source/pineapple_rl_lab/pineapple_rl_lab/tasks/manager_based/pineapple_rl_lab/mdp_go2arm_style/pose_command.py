...
from pineapple_rl_lab.tasks.manager_based.pineapple_rl_lab.agents_go2arm_style.rsl_rl_ppo_cfg_v2_5 import (
    Go2ArmFlatPPORunnerCfg,
    Go2ArmRoughPPORunnerCfg,
)
if TYPE_CHECKING:
    from pineapple_rl_lab.env_go2arm_style.manager_env import ManagerRLEnv as ManagerBasedRLEnv
    from pineapple_rl_lab.tasks.manager_based.pineapple_rl_lab.mdp_go2arm_style.cfg.command_cfg import (
...
class UniformPoseCommand(CommandTerm):
    ...
    def __init__(self, cfg: UniformPoseCommandCfg, env: ManagerBasedRLEnv):
        ...
        if self.cfg.is_Go2ARM_Flat == True:
            cfg_runner = Go2ArmFlatPPORunnerCfg()
            self.num_env_step = cfg_runner.num_steps_per_env
        else:
            cfg_runner = Go2ArmRoughPPORunnerCfg()
            self.num_env_step = cfg_runner.num_steps_per_env
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        def _get_invalid_pose_mask() -> torch.Tensor:
            pose_command_b = self.pose_command_b[env_ids_tensor]
            length_arm = torch.norm(pose_command_b[:, :3], dim=1)
            return (
                (length_arm > 0.7)
                | (length_arm < 0.3)
                # | ((pose_command_b[:, 0] < 0.35) & (torch.abs(pose_command_b[:, 1]) < 0.2))
            )
        ...
        if self.cfg.is_Go2ARM == True:
            ...
            invalid_pose = _get_invalid_pose_mask()
            while invalid_pose.any():
                resample_env_ids = env_ids_tensor[invalid_pose]
                r_resample = torch.empty(len(resample_env_ids), device=self.device)
                self.pose_command_b[resample_env_ids, 0] = (
                    r_resample.uniform_(*self.cfg.ranges_init.pos_x) * torch.clamp((1 - count), 0, 1)
                    + r_resample.uniform_(*self.cfg.ranges_final.pos_x) * torch.clamp((count), 0, 1)
                )
                self.pose_command_b[resample_env_ids, 1] = (
                    r_resample.uniform_(*self.cfg.ranges_init.pos_y) * torch.clamp((1 - count), 0, 1)
                    + r_resample.uniform_(*self.cfg.ranges_final.pos_y) * torch.clamp((count), 0, 1)
                )
                self.pose_command_w_z[resample_env_ids, 0] = (
                    r_resample.uniform_(*self.cfg.ranges_init.pos_z) * torch.clamp((1 - count), 0, 1)
                    + r_resample.uniform_(*self.cfg.ranges_final.pos_z) * torch.clamp((count), 0, 1)
                )
                self.pose_command_b[resample_env_ids, 2] = (
                    self.pose_command_w_z[resample_env_ids, 0] - self.robot.data.root_pos_w[resample_env_ids, 2]
                )
            ...
            euler_angles[:, 1] = r.uniform_(*self.cfg.ranges_init.pitch) * torch.clamp((1 - count), 0, 1) + \
                                 r.uniform_(*self.cfg.ranges_final.pitch) * torch.clamp((count), 0, 1)   
                                    
            euler_angles[:, 2] = r.uniform_(*self.cfg.ranges_init.yaw) * torch.clamp((1 - count), 0, 1) + \
            ...
            while invalid_pose.any():
                resample_env_ids = env_ids_tensor[invalid_pose]
                r_resample = torch.empty(len(resample_env_ids), device=self.device)
                self.pose_command_b[resample_env_ids, 0] = r_resample.uniform_(*self.cfg.ranges.pos_x)
                self.pose_command_b[resample_env_ids, 1] = r_resample.uniform_(*self.cfg.ranges.pos_y)
                self.pose_command_w_z[resample_env_ids, 0] = r_resample.uniform_(*self.cfg.ranges.pos_z)
                self.pose_command_b[resample_env_ids, 2] = (
                    self.pose_command_w_z[resample_env_ids, 0] - self.robot.data.root_pos_w[resample_env_ids, 2]
                )
