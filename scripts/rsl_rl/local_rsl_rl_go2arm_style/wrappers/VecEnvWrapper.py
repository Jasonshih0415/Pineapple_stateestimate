...
from pineapple_rl_lab.env_go2arm_style.manager_env import ManagerRLEnv 
...
class RslRlVecEnvWrapper(VecEnv):
    ...
    def __init__(self, env: ManagerRLEnv | DirectRLEnv, clip_actions: float | None = None):
        ...
        if not isinstance(env.unwrapped, ManagerRLEnv) and not isinstance(env.unwrapped, DirectRLEnv):
            raise ValueError(
                "The environment must be inherited from ManagerRLEnv or DirectRLEnv. Environment type:"
                f" {type(env)}"
            )
        ...
        if hasattr(self.unwrapped, "observation_manager"):
            ...
            self.num_history, self.num_prop, self.num_priv = self.unwrapped.observation_manager.compute_obs()
