...
Pineapple V2 kinematics are configured by :meth:`UsePineappleV2` as two
wheel-leg contact chains.
"""
...
MAX_CONTACT_CHAIN = 2
# hip, thigh and calf; the wheel is handled as a rolling constraint.
MAX_CHAIN_NODE = 3
...
class SensorLegsPos(Sensors):
    ...
    # ==================================================================
    # Pineapple V2 kinematic model
    # ==================================================================
    def _reset_chains(self):
        self.LegChains_ = [LegTFChain() for _ in range(MAX_CONTACT_CHAIN)]

    def UsePineappleV2(self):
        """Configure two wheel-leg chains from ``pineapple_v2.xml``.

        Joint slots use robot/XML order: left hip/thigh/calf/wheel at
        0..3 and right hip/thigh/calf/wheel at 4..7. CAPO's joint message
        stores q at 0..15, dq at +16 and tau at +32.
        """
        self._reset_chains()
        self.ContactChainNum = 2
        self.Environement_Height_Scope = 0.08
        ...
        specs = (
            (0, (0.086334, 0.050823, 0.056),
             (-0.066166, 0.063177, 0.0),
             (0.0, 0.056, -0.19),
             (0.0, 0.0355, -0.19)),
            (4, (0.086334, -0.050823, 0.056),
             (-0.066166, -0.063177, 0.0),
             (0.0, -0.056, -0.19),
             (0.0, -0.0355, -0.19)),
        )

        for leg_number, (base, hip_t, thigh_t, calf_t, wheel_t) in enumerate(specs):
            chain = self.LegChains_[leg_number]
            chain.node_num = 3
            chain.node[0] = TFNode(
                -1, base, 16 + base, 32 + base, TF_AXIS_X, *hip_t)
            chain.node[1] = TFNode(
                0, base + 1, 17 + base, 33 + base, TF_AXIS_Y, *thigh_t)
            chain.node[2] = TFNode(
                1, base + 2, 18 + base, 34 + base, TF_AXIS_Y, *calf_t)
            chain.ee = TFNode(
                2, -1, -1, -1, TF_AXIS_FIXED, *wheel_t)

            chain.wheel_radius = 0.077
            chain.wheel_q_index = base + 3
            chain.wheel_dq_index = 19 + base
            chain.pitch_joint_num = 2
            chain.pitch_q_index[0:2] = [base + 1, base + 2]
            chain.pitch_dq_index[0:2] = [17 + base, 18 + base]
            chain.roll_joint_num = 1
            chain.roll_q_index[0] = base
    ...
    def SensorDataHandle(self, message, time):
        ...
        # 1. cal each chain wheel q dq tai get pos,vel,contact
        ...
        active_contacts = self.FootIsOnGround[:self.ContactChainNum]
        if any(active_contacts):
            H = ssm.Matrix_H
            for i in range(9):
                H[i, i] = 0.0
            # 2. cal body pos, vel
            ...
            self.FootfallAveragePosition[0] = 0.5 * (p_w[0][0] + p_w[1][0])
            self.FootfallAveragePosition[1] = 0.5 * (p_w[0][1] + p_w[1][1])

            # Right->left is the body lateral direction. Rotating it by
            # -90 degrees gives the body heading.
            lateral_x = p_w[0][0] - p_w[1][0]
            lateral_y = p_w[0][1] - p_w[1][1]
            yaw_ff = math.atan2(lateral_y, lateral_x) - 0.5 * M_PI
    ...
    def LoadedWeightCheck(self, message, time):
        ...
        active_probabilities = self.FootfallProbability[:self.ContactChainNum]
        # Preserve the original four-leg criterion (2.5 / 4 = 0.625), but
        # scale it with the number of active contact chains.
        all_on_ground = (
            self.ContactChainNum > 0
            and sum(active_probabilities) > 0.625 * self.ContactChainNum
        )
        ...
        if self._stable_cnt >= STABLE_N:
            fz_sum = sum(
                self.FootBodyEff_WF[i][2]
                for i in range(self.ContactChainNum)
            )
    ...
    # ==================================================================
    # state update: FootBodyPos_WF, FootBodyVel_WF, FootBodyEff_wf(force)
    #               FootfallProbability,FootIsOnGround,FootLanding,Observation(velocity)     
    def Joint2HipFoot(self, message, LegNumber):
        ...
        # determine the foot whether on ground by it's force
    ...
    def FootFallPositionRecord(self, message):
        ...
        for LegNumber in range(self.ContactChainNum):
            ...
            if not self.FootfallPositionRecordIsInitiated[LegNumber]:
                ...
                rec[0] = ssm.EstimatedState[0] + foot[0]
                rec[1] = ssm.EstimatedState[3] + foot[1]
                # ``foot`` is the wheel-hub position.  The height anchor,
                # however, represents the wheel-ground contact point, which
                # is one wheel radius below the hub on level ground.
                rec[2] = (
                    ssm.EstimatedState[6] + foot[2] - chain.wheel_radius
                )
                rec[3] = self.ObservationTime
                self._update_shank_prev(message, LegNumber, body_pitch, body_roll)
            ...
            # Recover base height from the contact point rather than from
            # the wheel hub.  Without this offset CAPO underestimates Z by
            # one wheel radius (0.077 m for Pineapple V2).
            p_sum[2] += rec[2] - foot[2] + chain.wheel_radius
        ...
        # SensorDataHandle normally calls this only with an active contact,
        # but contact state may change while processing noisy measurements.
        if leg_cnt == 0:
            return False
