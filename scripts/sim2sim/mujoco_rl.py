...
def read_sensors(data: mujoco.MjData, config: SimConfig, include_sipo: bool = False):
    num_actions = config.num_actions
    ...
    imu_acc = data.sensordata[3 * num_actions + 7 : 3 * num_actions + 10] if include_sipo else None
    return qpos, qvel, imu_quat, ang_vel_b, imu_acc, lin_vel_i
...
def run_simulation(
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running() and counter * config.simulation_dt < config.simulation_duration:
            ...
            qpos, qvel, imu_quat, ang_vel_b, imu_acc, lin_vel_i = read_sensors(
                data,
                config,
                include_sipo=sipo_runner is not None,
            )
    ...
    return buffers, sipo_runner.buffers if sipo_runner is not None else None
